...
def ret_weight_name(symbol, k, pr_k, m_lis, y_mode='binary'):
    ...
    weight_name = 'weights/{}/{}/k{}_prk{}_mlis{}'.format(
        y_mode, symbol, k, pr_k, st)
...
class ModelTrainer:
    ...
    def run_train(self, seq_len, weights_save_name, batch_size,
        ...
        for repeat in range(repeats):
            if y_mode == 'binary':
                model = liza_models.LizaTransformer(seq_len, out_dim=2)
                # データとモデルを用いてトレーニングのセッションを初期化
                liza_trainer = trainer.LizaTrainerBinary(model, weight_name, batch_size,
                                                         self.hist, self.m_lis, self.k, self.pr_k)
            elif y_mode == 'contrarian':
                model = liza_models.LizaTransformer(seq_len, out_dim=3)
                # データとモデルを用いてトレーニングのセッションを初期化
                liza_trainer = trainer.LizaTrainerContrarian(model, weight_name, batch_size,
                                                             self.hist, self.m_lis, self.k, self.pr_k)
...
k = 12
pr_k = 12
for k, batch_size in [[12, 120*1000], [24, 120*1000]]:
